...
void printlword (uint64_t lword);
...
pool_s *pool_s_ (uint16_t csize) 
{
  ...
  pool = calloc(1, sizeof(*pool) + _POOLSIZE*((csize >> 3) + (csize % 8 != 0)));
  if (!pool)
    goto err_;
  pool->chromsize = csize;
  pool->cmask = csize % 64;
  return pool;
...
pool_s *pool_init (wgraph_s *g)
{
  ...
  if (!pool)
    return NULL;
  n = (pool->chromsize/sizeof(int)) + (pool->chromsize % sizeof(int) != 0);
  for (i = 0; i < _POOLSIZE; i++) {
    for (j = 0; j < n; j++)
      *((int *)(pool->popul[i])+j) = rand();
  }
  return pool;
}
...
void printlword (uint64_t lword)
{
  ...
  for (i = 0; i < 64; i++)
    printf("%d",(int)(((lword << i) & 0x8000000000000000) >> 63));
  printf("\n");
}

void printchrom (pool_s *p, uint16_t index)
{
  uint16_t i, n;
  uint64_t *ptr;
  
  n = (p->chromsize >> 3) + (p->chromsize % 8 == 0);
  ptr = p->popul + index*n;
  for (i = 0; i < n; i++) {
    if (i == n-1)
      printchrom (p, ptr[i]);
